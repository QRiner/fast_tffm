...
class FmScorerOpTest(tf.test.TestCase):
    def testBasic(self):
        ...
        tf_pred = 0.5 * tf.reduce_sum(factor_sum * factor_sum, [1]) - 0.5 * tf.reduce_sum(
            fvals * fvals * factors * factors, [1, 2]) + tf.reduce_sum(biases * feature_vals, [1]);
        ...
        my_pred, my_reg = fm_ops.fm_scorer(flat_feature_ids, params_var, flat_feature_vals, flat_feature_poses,
